...
import { ProgramFormValue } from '../../models/programs.models';
...
@Component({
  ...
  styleUrl: './program-form.component.less',
})
export class ProgramFormComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const request$ =
      this.isEditMode() && this.editSlug != null
        ? this.programsService.patch(this.editSlug, body)
        : this.programsService.create(body as ProgramFormValue);
      ...
      next: (res) => {
        this.message.success(
          this.translate.instant(
            this.isEditMode()
              ? 'ADMIN.PROGRAMS.MESSAGES.UPDATE_SUCCESS'
              : 'ADMIN.PROGRAMS.MESSAGES.CREATE_SUCCESS'
          )
        );
        this.submitting.set(false);
        void this.router.navigate(['/admin/programs', res.slug]);
      },
      error: (error) => {
        this.submitting.set(false);
  ...
  private buildBody(): ProgramFormValue {
