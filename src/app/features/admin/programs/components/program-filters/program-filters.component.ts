...
import { ProgramFilters } from '../../models/programs.models';
import {
...
@Component({
  ...
  template: `
    <div class="program-filters">
      <div class="admin-filters-bar">
        <nz-input-group [nzPrefix]="searchIcon">
          <input
            nz-input
            type="text"
            [placeholder]="'ADMIN.PROGRAMS.SEARCH_PLACEHOLDER' | translate"
            [ngModel]="searchValue"
        ...
        >
          ...
          <span>{{ 'ADMIN.PROGRAMS.FILTERS.BUTTON' | translate }}</span>
        </button>
      </div>
      ...
      <nz-modal
        ...
        [nzTitle]="'ADMIN.PROGRAMS.FILTERS.MODAL_TITLE' | translate"
        [nzWidth]="'min(560px, 92vw)'"
        nzCentered
      >
        <ng-container *nzModalContent>
          <div class="program-filters__modal-grid">
            <nz-select
              class="program-filters__select"
              [nzPlaceHolder]="'ADMIN.PROGRAMS.FILTERS.PUBLISHER' | translate"
              nzSize="default"
              nzAllowClear
              nzShowSearch
              nzServerSearch
            ...
            <nz-select
              class="program-filters__select"
              [nzPlaceHolder]="'ADMIN.PROGRAMS.FILTERS.LICENSE' | translate"
              nzSize="default"
              nzAllowClear
            ...
            <nz-select
              class="program-filters__select"
              [nzPlaceHolder]="'ADMIN.PROGRAMS.FILTERS.LANGUAGE' | translate"
              nzSize="default"
              nzAllowClear
            ...
            <nz-select
              class="program-filters__select"
              [nzPlaceHolder]="'ADMIN.PROGRAMS.FILTERS.IS_EXTERNAL' | translate"
              nzSize="default"
              nzAllowClear
            ...
            >
              <nz-option
                [nzValue]="true"
                [nzLabel]="'ADMIN.PROGRAMS.FILTERS.TYPE_EXTERNAL' | translate"
              ></nz-option>
              <nz-option
                [nzValue]="false"
                [nzLabel]="'ADMIN.PROGRAMS.FILTERS.TYPE_INTERNAL' | translate"
              ></nz-option>
            </nz-select>
          </div>
        </ng-container>
        <ng-container *nzModalFooter>
          <div class="program-filters__modal-footer">
            <button nz-button nzType="default" nzSize="default" (click)="clearAdvancedFilters()">
              {{ 'ADMIN.PROGRAMS.FILTERS.CLEAR' | translate }}
            </button>
            <button nz-button nzType="primary" nzSize="default" (click)="closeFiltersModal()">
              {{ 'ADMIN.PROGRAMS.FILTERS.DONE' | translate }}
            </button>
          </div>
        </ng-container>
      </nz-modal>
  ...
  `,
  styleUrl: './program-filters.component.less',
})
export class ProgramFiltersComponent implements OnInit {
  @Output() filtersChange = new EventEmitter<Partial<ProgramFilters>>();
  initialFilters = input<Partial<ProgramFilters>>({});
  ...
  private currentFilters: Partial<ProgramFilters> = {};
  ...
  private hydrateFromFilters(f: Partial<ProgramFilters>): void {
