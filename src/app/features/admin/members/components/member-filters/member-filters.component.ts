...
import { MemberStatus } from '../../models/members.models';

export interface MemberFiltersPayload {
  search?: string;
  status?: MemberStatus;
}
...
@Component({
      ...
      <div class="admin-filters-bar">
        <nz-input-group [nzPrefix]="searchIcon">
          <input
            nz-input
            type="text"
            [placeholder]="'ADMIN.MEMBERS.SEARCH_PLACEHOLDER' | translate"
            [ngModel]="searchValue"
        ...
        <nz-select
          class="member-filters__select member-filters__select--status"
          nzAllowClear
          nzShowArrow
          [nzPlaceHolder]="'ADMIN.MEMBERS.FILTERS.STATUS' | translate"
          nzSize="default"
  ...
  `,
  styleUrl: './member-filters.component.less',
})
export class MemberFiltersComponent {
  @Output() readonly filtersChange = new EventEmitter<MemberFiltersPayload>();
  ...
  readonly statusOptions: MemberStatus[] = ['pending', 'active'];
  ...
  statusLabel(s: MemberStatus): string {
    return this.translate.instant(`ADMIN.MEMBERS.STATUS.${s.toUpperCase()}`);
  }

  private hydrate(raw: Record<string, string | number | undefined>): void {
    this.searchValue = raw['search'] != null ? String(raw['search']) : '';
    const st = raw['status'];
    this.selectedStatus = st === 'pending' || st === 'active' ? st : null;
  }
  ...
  private emit(partial: Partial<MemberFiltersPayload>): void {
    this.filtersChange.emit({
