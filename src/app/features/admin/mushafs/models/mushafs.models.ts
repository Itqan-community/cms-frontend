...
/** GET /portal/mushafs/{mushaf_slug}/ */
export interface MushafDetails {
  id: number;
  slug: string;
  name_ar: string;
  name_en: string;
...
export interface MushafsList {
  results: MushafItem[];
  count: number;
}

export interface MushafFilters {
  page: number;
  page_size: number;
  search?: string;
  publisher_id?: number;
...
export interface MushafFormValue {
  name_ar: string;
  name_en: string;
  description_ar: string;
  description_en: string;
