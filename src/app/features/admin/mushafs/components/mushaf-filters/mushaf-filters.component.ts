...
import { MushafFilters } from '../../models/mushafs.models';
import {
...
@Component({
  ...
  template: `
    <div class="mushaf-filters">
      <div class="admin-filters-bar">
        <nz-input-group [nzPrefix]="searchIcon">
          <input
            nz-input
            type="text"
            [placeholder]="'ADMIN.MUSHAFS.SEARCH_PLACEHOLDER' | translate"
            [ngModel]="searchValue"
        ...
        >
          ...
          <span>{{ 'ADMIN.MUSHAFS.FILTERS.BUTTON' | translate }}</span>
        </button>
      </div>
      ...
      <nz-modal
        ...
        [nzTitle]="'ADMIN.MUSHAFS.FILTERS.MODAL_TITLE' | translate"
        [nzWidth]="'min(560px, 92vw)'"
        nzCentered
      >
        <ng-container *nzModalContent>
          <div class="mushaf-filters__modal-grid">
            <nz-select
              class="mushaf-filters__select"
              [nzPlaceHolder]="'ADMIN.MUSHAFS.FILTERS.PUBLISHER' | translate"
              nzSize="default"
              nzAllowClear
              nzShowSearch
              nzServerSearch
            ...
            <nz-select
              class="mushaf-filters__select"
              [nzPlaceHolder]="'ADMIN.MUSHAFS.FILTERS.LICENSE' | translate"
              nzSize="default"
              nzAllowClear
            ...
            <nz-select
              class="mushaf-filters__select"
              [nzPlaceHolder]="'ADMIN.MUSHAFS.FILTERS.LANGUAGE' | translate"
              nzSize="default"
              nzAllowClear
            ...
            <nz-select
              class="mushaf-filters__select"
              [nzPlaceHolder]="'ADMIN.MUSHAFS.FILTERS.IS_EXTERNAL' | translate"
              nzSize="default"
              nzAllowClear
            ...
            >
              <nz-option
                [nzValue]="true"
                [nzLabel]="'ADMIN.MUSHAFS.FILTERS.TYPE_EXTERNAL' | translate"
              ></nz-option>
              <nz-option
                [nzValue]="false"
                [nzLabel]="'ADMIN.MUSHAFS.FILTERS.TYPE_INTERNAL' | translate"
              ></nz-option>
            </nz-select>
          </div>
        </ng-container>
        <ng-container *nzModalFooter>
          <div class="mushaf-filters__modal-footer">
            <button nz-button nzType="default" nzSize="default" (click)="clearAdvancedFilters()">
              {{ 'ADMIN.MUSHAFS.FILTERS.CLEAR' | translate }}
            </button>
            <button nz-button nzType="primary" nzSize="default" (click)="closeFiltersModal()">
              {{ 'ADMIN.MUSHAFS.FILTERS.DONE' | translate }}
            </button>
          </div>
        </ng-container>
      </nz-modal>
  ...
  `,
  styleUrl: './mushaf-filters.component.less',
})
export class MushafFiltersComponent implements OnInit {
  @Output() filtersChange = new EventEmitter<Partial<MushafFilters>>();
  initialFilters = input<Partial<MushafFilters>>({});
  ...
  private currentFilters: Partial<MushafFilters> = {};
  ...
  private hydrateFromFilters(f: Partial<MushafFilters>): void {
