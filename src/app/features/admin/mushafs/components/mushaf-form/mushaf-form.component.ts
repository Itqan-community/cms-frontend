...
import { MushafFormValue } from '../../models/mushafs.models';
import { MushafsService } from '../../services/mushafs.service';

@Component({
  selector: 'app-mushaf-form',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    RouterLink,
...
export class MushafFormComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const request$ =
      this.isEditMode() && this.editSlug != null
        ? this.mushafsService.patch(this.editSlug, body)
        : this.mushafsService.create(body as MushafFormValue);
      ...
      next: (res) => {
        this.message.success(
          this.translate.instant(
            this.isEditMode()
              ? 'ADMIN.MUSHAFS.MESSAGES.UPDATE_SUCCESS'
              : 'ADMIN.MUSHAFS.MESSAGES.CREATE_SUCCESS'
          )
        );
        this.submitting.set(false);
        void this.router.navigate(['/admin/mushafs', res.slug]);
      },
      error: (error) => {
        this.submitting.set(false);
